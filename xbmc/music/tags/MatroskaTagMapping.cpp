         ...
         CMusicInfoTag& tag)
{
  ...
  */
  if (key == "ALBUM")
    ...
    // SetArtist() splits on musicsep, so several artists have to arrive joined with it. Splitting
    // on separators first would take "/" along and make two artists out of AC/DC.
    tag.SetArtist(StringUtils::Join(
        StringUtils::Split(value, MatroskaTagMapping::MultiValueSeparator), musicsep));
  else if (key == "ARTISTS")
    tag.SetMusicBrainzArtistHints(SplitValues(value, separators));
  ...
  // true trims any whitespace around the genre(s)
    ...
    tag.SetArtistSort(StringUtils::Join(SplitValues(value, separators), musicsep));
  else if (key == "ALBUMARTISTSORT" || key == "SORT_ALBUM_ARTIST" || key == "ALBUM ARTIST SORT")
    tag.SetAlbumArtistSort(StringUtils::Join(SplitValues(value, separators), musicsep));
  else if (key == "COMPOSERSORT")
    tag.SetComposerSort(StringUtils::Join(SplitValues(value, separators), musicsep));
  ...
  else if (key == "COPYRIGHT")
  {
    ...
    tag.AddArtistRole("Writer", SplitValues(value, separators));
  ...
  else if (key == "ARRANGER")
  {
    ...
    tag.AddArtistRole("Remixer", SplitValues(value, separators));
  else if (key == "MIXED_BY" || key == "MIXER")
    tag.AddArtistRole("Mixer", SplitValues(value, separators));
  else if (key == "LYRICIST")
    tag.AddArtistRole("Lyricist", SplitValues(value, separators));
  else if (key == "COMPOSER")
    tag.AddArtistRole("Composer", SplitValues(value, separators));
  else if (key == "CONDUCTOR")
    tag.AddArtistRole("Conductor", SplitValues(value, separators));
  else if (key == "ENGINEER")
    tag.AddArtistRole("Engineer", SplitValues(value, separators));
  else if (key == "PRODUCER")
    tag.AddArtistRole("Producer", SplitValues(value, separators));
  else if (key == "BAND")
    tag.AddArtistRole("Band", SplitValues(value, separators));
