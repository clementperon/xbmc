...
MatroskaAlbum ReadWithFFmpegImpl(const AVFormatContext* fctx)
{
  ...
  for (unsigned int i = 0; fctx->chapters && i < fctx->nb_chapters; ++i)
  {
     ...
     * precedes it - ends of no length, which IsTrack() then drops. A file declaring neither its
     * Duration nor any ChapterTimeEnd is therefore not expanded on this reader, where TagLib
     * expands it - see TestMatroskaTagReaderFFmpeg.ClampsEveryEndOfAFileThatDeclaresNoLength.
     *
     * Zero is how an unclosed chapter reaches IsTrack() and CAudioBookFileDirectory, which serve
     * either reader; the guard below is what keeps a negative end from ever spelling one.
     */
