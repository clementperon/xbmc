                                       ...
                                       EmbeddedArt* art)
{
  ...
  */
  ...
  for (const auto& t : album.fileTags)
    CMatroskaTagParser::ParseTag(t.first, t.second, separators, musicsep, tag);
...
// The TagLib reader below, and only it, needs headers that do not exist before TagLib 2.2.
// See TagLibVersion.h for why the floor is 2.3.1. The loader above reads through whichever
// reader the build has, so it is built either way.
