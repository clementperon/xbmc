...
{
  CMusicInfoTag tag;
  CMatroskaTagParser::ParseTag(key, value, Separators, MusicSep, tag);
  return tag;
}
...
bool HasRole(const CMusicInfoTag& tag, const std::string& role, const std::string& artist)
{
  ...
  return std::any_of(contributors.begin(), contributors.end(),
                     [&](const CMusicRole& c)
 ...
 * field as it does for the TagLib reader. Both call ParseTag, so this table is the whole contract
 * between them.
 */
...
TEST(TestMatroskaTagParser, MapsDatesToReleaseAndOriginal)
{
...
TEST(TestMatroskaTagParser, MapsMusicBrainzIdentifiers)
{
...
TEST(TestMatroskaTagParser, MapsRolesToContributors)
{
 ...
 */
TEST(TestMatroskaTagParser, SplitsCommaDelimitedRolePairs)
{
...
TEST(TestMatroskaTagParser, IgnoresATrailingRoleWithNoPerson)
{
...
TEST(TestMatroskaTagParser, SplitsMultiValueTagsOnTheSeparatorList)
{
...
TEST(TestMatroskaTagParser, SplitsGenreOnTheMusicItemSeparator)
{
 ...
 */
TEST(TestMatroskaTagParser, MatchesUpperCaseKeysOnly)
{
...
TEST(TestMatroskaTagParser, LeavesTheTagUntouchedForUnknownKeys)
{
  ...
  CMatroskaTagParser::ParseTag("NOT_A_MATROSKA_TAG", "junk", Separators, MusicSep, tag);
 ...
 * dropped rather than thrown out of ParseTag.
 */
TEST(TestMatroskaTagParser, SurvivesNonNumericTrackAndDiscNumbers)
{
  EXPECT_NO_THROW({
