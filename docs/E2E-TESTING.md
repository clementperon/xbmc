...
> Status: proposal, with an initial proof-of-concept slice implemented. This document
...
> The rest of this document remains the broader plan the POC is the first step of.
...
flowchart TB
    ...
    subgraph launchers [Platform-specific launchers]
        linuxLauncher["Linux launcher: Xvfb + binary"]
        macLauncher["macOS launcher: .app bundle"]
        winLauncher["Windows launcher: kodi.exe"]
        androidLauncher["Android launcher: adb + emulator (future)"]
        iosLauncher["iOS/tvOS launcher: simulator (future)"]
    end
    ...
    androidLauncher -.future.-> jsonrpcClient
    iosLauncher -.future.-> jsonrpcClient
...
- `linux` job: build (or reuse an existing build job/nightly artifact) on
  `ubuntu-latest`, run the driver under `xvfb-run`, upload logs/screenshots.
...
- **Android**: `reactivecircus/android-emulator-runner` (or a self-hosted runner) GitHub
  Actions job, install the debug APK, `adb forward` the JSON-RPC port to localhost, run
  the same `scenarios/` suite unmodified. Add Espresso/UIAutomator only for
  Android-chrome concerns (permission dialogs, PiP, back button) that JSON-RPC can't
  see.
- **iOS/tvOS**: macOS runner + Simulator, launch via `xcrun simctl`, JSON-RPC over
  loopback. Add XCUITest only for OS-chrome concerns.
