...
> the JSON-RPC webserver responds, and shut it down cleanly. See
> `docs/E2E-TESTING.md` for the full phased plan.

## What this does

`scenarios/test_startup.py` launches a built `kodi.bin`/`kodi` binary with a throwaway
`portable_data` profile (`-p`), pre-seeded with the webserver enabled on a non-default
port and authentication disabled. It then:
...
- No test media / playback testing yet (ping + quit only).
